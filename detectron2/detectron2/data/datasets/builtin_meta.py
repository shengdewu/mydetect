...
def _get_xintu_instances_meta():
    thing_ids = [k["id"] for k in XINTU_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in XINTU_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 1, len(thing_ids)
    # Mapping from the incontiguous COCO category id to an id in [0, 79]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in XINTU_CATEGORIES if k["isthing"] == 1]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
        "thing_colors": thing_colors,
...
def _get_builtin_metadata(dataset_name):
    ...
    elif dataset_name == 'xintu':
        return _get_xintu_instances_meta()
